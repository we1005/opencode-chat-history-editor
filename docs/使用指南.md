...
- 搜索结果可通过“查看上下文”定位到完整消息流中的原始位置；保留搜索词，支持返回搜索结果和重新定位。
...
搜索只显示命中消息时，每条结果旁会提供 **查看上下文** 按钮：

1. 点击后恢复完整消息流，按消息 ID 定位并高亮目标，显示前后消息。
2. 顶部保留搜索词，并提示正在查看哪一条消息的上下文及完整消息数量。
3. 点击 **返回搜索结果** 恢复过滤结果并定位到刚才的命中消息；也可以修改搜索词发起新搜索。
4. 在上下文中浏览较远后，用 **回到定位消息** 回到目标位置。

长距离跳转直接定位，避免滚动经过数百条消息；邻近跳转使用平滑滚动，并尊重系统的减少动画设置。
...
```bash
npm test                  # 编辑服务、真实组件渲染与搜索上下文交互回归测试
