...
export function SessionDetailPanel({ session, onDelete }: SessionDetailPanelProps) {
  ...
  const visible = useMemo(() => (messages || []).map((message, index) => ({ message, index })).filter(({ message }) =>
    !messageSearch || (message.data + (message.parts || []).map(part => part.data).join('\n')).toLowerCase().includes(messageSearch.toLowerCase())), [messages, messageSearch]);

  function jump(index: number) {
    setMessageSearch(''); setNavOpen(false);
  ...
  return <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
    ...
    <Box sx={{ px: 2, py: 1.5, borderBottom: 1, borderColor: 'divider' }}>
      <TextField fullWidth size="small" label="搜索正文、思考、工具、错误和元数据" value={messageSearch} onChange={event => setMessageSearch(event.target.value)} />
      ...
      {!!messageSearch && <Typography variant="caption" color="text.secondary">匹配 {visible.length} / {messages?.length || 0} 条消息</Typography>}
    </Box>
    <Box data-message-scroll sx={{ flex: 1, overflow: 'auto', overflowX: 'hidden', minHeight: 0 }}>
      {isLoading ? <Loading message="加载消息..." /> : error ? <Alert severity="error" sx={{ m: 2 }}>{error.message}</Alert> : visible.length ?
        <List disablePadding>{visible.map(({ message, index }) => <MessageItem key={message.id} message={message} index={index} registerRef={registerRef} highlightedIndex={highlightedIndex} expandTools={expandTools} />)}</List> :
    ...
    <Dialog open={navOpen} onClose={() => setNavOpen(false)} maxWidth="sm" fullWidth>
      ...
      <DialogContent dividers sx={{ p: 0 }}><List dense>{userMessages.map(item => <ListItemButton key={item.message.id} onClick={() => jump(item.index)}>
